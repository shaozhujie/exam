...
public class ApeTestController {
    ...
    public Result saveApeTest(@RequestBody ApeTest apeTest) {
        ...
        //判断类型
        int total = 0;
        if (apeTest.getFlag() == 0) {
            QueryWrapper<ApeTaskTest> queryWrapper = new QueryWrapper<>();
            queryWrapper.lambda().eq(ApeTaskTest::getTaskId,apeTest.getTaskId());
            List<ApeTaskTest> testList = apeTaskTestService.list(queryWrapper);
            Set<Integer> numbers = new HashSet<>();
            while(numbers.size() < apeTest.getNum()) {
                for (int i = 0; i < apeTest.getNum(); i++) {
            ...
            for (Integer number : numbers) {
                ApeTaskTest apeTaskTest = testList.get(number);
                ApeTestItem apeTestItem = new ApeTestItem();
                apeTestItem.setItemId(apeTaskTest.getId());
                apeTestItem.setTestId(idStr);
                apeTestItem.setTag(apeTaskTest.getTag());
                ...
                total += apeTaskTest.getScore();
                apeTestItemService.save(apeTestItem);
            }
            apeTest.setTotalScore(total);
        }
    ...
    @PostMapping("getTestStudentList")
    public Result getTestStudentList(@RequestBody JSONObject jsonObject) {
        String testId = jsonObject.getString("testId");
        QueryWrapper<ApeTestStudent> queryWrapper =  new QueryWrapper<>();
        queryWrapper.lambda().eq(ApeTestStudent::getTestId,testId)
                .groupBy(ApeTestStudent::getUserId).orderByAsc(ApeTestStudent::getUpdateTime);
        ...
        for (ApeTestStudent student : studentPage) {
            ...
            student.setTotalGetScore(score);
            names.add(student.getCreateBy());
            nums.add(student.getTotalGetScore());
        }
